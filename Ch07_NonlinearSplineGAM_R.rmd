...
model <- lm(Outstate ~  Apps + Accept + Enroll + Top10perc, data = train_data)

summary(model)

predictions <- predict(model, newdata = test_data)

rmse <- sqrt(mean((predictions - test_data$Outstate)^2))
rmse
```
...
```{r,echo=TRUE}
library(mgcv)
gam_model <- gam(Outstate ~  s(Apps) + s(Accept) + s(Enroll) + s(Top10perc), data = train_data)
...
rsquared <- cor(predictions, test_data$Outstate)^2
...
gam_fit <- gam(mpg ~ s(horsepower), data = Auto)

par(mfrow = c(2, 2))
plot(Auto$horsepower, Auto$mpg, xlab = "Horsepower", ylab = "MPG", main = "Scatterplot with Linear Fit")
abline(lm_fit, col = "red")

plot(Auto$horsepower, Auto$mpg, xlab = "Horsepower", ylab = "MPG", main = "Scatterplot with Quadratic Fit")
points(Auto$horsepower, predict(quad_fit), col = "blue", pch = 20)

plot(Auto$horsepower, Auto$mpg, xlab = "Horsepower", ylab = "MPG", main = "Scatterplot with Cubic Fit")
points(Auto$horsepower, predict(cubic_fit), col = "green", pch = 20)

legend("topright", legend = c("Data", "Linear Fit", "Quadratic Fit", "Cubic Fit"),
       col = c("black", "red", "blue", "green"), pch = c(1, NA, 20, 20), lty = c(NA, 1, NA, NA))

plot(gam_fit, se = TRUE, col = "blue", main = "GAM Fit: MPG ~ Horsepower")
```
...
beta1 <- 2  # Initialize to a random value
...
for (i in 1:1000) {
  ...
  beta_values[i, ] <- c(coef(model)[1], beta1, beta2)
}

plot(1:1000, beta_values[, 1], type = 'l', col = 'red', xlab = 'Iteration', ylab = 'Coefficient Estimate', main = 'Estimates of beta0, beta1, beta2')
lines(1:1000, beta_values[, 2], col = 'blue')
lines(1:1000, beta_values[, 3], col = 'green')
...
lines(1:1000, beta_values[, 2], col = 'blue')
lines(1:1000, beta_values[, 3], col = 'green')
abline(h = coef(lm_model)[1], col = 'red', lty = 2)
abline(h = coef(lm_model)[2], col = 'blue', lty = 2)
abline(h = coef(lm_model)[3], col = 'green', lty = 2)
legend('topright', legend = c('beta0', 'beta1', 'beta2', 'lm beta0', 'lm beta1', 'lm beta2'), col = c('red', 'blue', 'green', 'red', 'blue', 'green'), lty = c(1, 1, 1, 2, 2, 2))
...
distances <- apply(beta_values, 1, function(row) euclidean_distance(row, coef(lm_model)))

tolerance <- 0.01  
iterations_to_convergence <- which.min(distances < tolerance)

print(iterations_to_convergence)
...
# Perform backfitting iterations
beta_values <- matrix(NA, nrow = 1000, ncol = p)

for (i in 1:1000) {
  for (j in 1:p) {
    a <- Y - X[, -j] %*% beta_values[i, -j]
    model <- try(lm(a ~ X[, j]), silent = TRUE)
    if (!inherits(model, "try-error")) {
      beta_values[i, j] <- coef(model)[2]
    } else {
      beta_values[i, j] <- NA
    }
  }
...
# Calculate Euclidean distance between backfitting coefficients and true coefficients
distances <- apply(beta_values, 1, function(row) sqrt(sum((row - beta_true)^2)))

# Find the iteration where the distance becomes negligible
tolerance <- 0.01  # Define a tolerance level
iterations_to_convergence <- which.min(distances < tolerance)

# Plot the convergence if there are finite values to plot
if (any(is.finite(distances))) {
  plot(1:1000, distances, type = "l", xlab = "Iteration", ylab = "Distance to True Coefficients", main = "Convergence of Backfitting Coefficients")
...
plot(Boston$dis, Boston$nox, xlab = "Distance to Employment Centers", ylab = "NOx Concentration", main = "Cubic Polynomial Regression", col = "blue")
...
plot(Boston$dis, Boston$nox, xlab = "Distance to Employment Centers", ylab = "NOx Concentration", main = "Polynomial Regression", col = "blue")

for (deg in 1:10) {
  fit <- lm(nox ~ poly(dis, deg, raw = TRUE), data = Boston)
  ...
  lines(sort(Boston$dis), fitted(fit)[order(Boston$dis)], col = rainbow(10)[deg], lwd = 2)
}

cat("Polynomial Degree | Residual Sum of Squares\n")
for (deg in 1:10) {
  cat(sprintf("%17d | %f\n", deg, rss_values[deg]))
}
```
...
for (deg in 1:10) {
  formula <- as.formula(paste("nox ~ poly(dis, ", deg, ", raw = TRUE)"))
  ...
  # Store the mean squared error (MSE) for the current degree
  mse_values[deg] <- cv_model$results$RMSE
}

plot(1:10, mse_values, type = "b", xlab = "Degree of Polynomial", ylab = "Cross-Validation RMSE", main = "Cross-Validation for Polynomial Degree Selection")
...
plot(Boston$dis, Boston$nox, xlab = "Distance to Employment Centers", ylab = "NOx Concentration", main = "Regression Spline Fit", col = "blue")
lines(sort(Boston$dis), predict(fit, data.frame(dis = sort(Boston$dis))), col = "red", lwd = 2)
```
...
plot(Boston$dis, Boston$nox, xlab = "Distance to Employment Centers", ylab = "NOx Concentration", main = "Regression Splines for Different Degrees of Freedom", col = "blue")
...
for (i in 1:length(df_range)) {
  ...
  lines(sort(Boston$dis), predict(fit, data.frame(dis = sort(Boston$dis))), col = rainbow(length(df_range))[i], lwd = 2)
}

cat("Degrees of Freedom | Residual Sum of Squares\n")
for (i in 1:length(df_range)) {
  cat(sprintf("%18d | %f\n", df_range[i], rss_values[i]))
}
```
...
for (i in 1:length(df_range)) {
  formula <- as.formula(paste("nox ~ bs(dis, df = ", df_range[i], ")"))
  ...
  mse_values[i] <- cv_model$results$RMSE
}

plot(df_range, mse_values, type = "b", xlab = "Degrees of Freedom", ylab = "Cross-Validation RMSE", main = "Cross-Validation for Regression Spline Degree Selection")

optimal_df <- df_range[which.min(mse_values)]
